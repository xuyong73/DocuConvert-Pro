...
namespace DocuConvertProRefactored.Services
{
    public partial class MDConvServices : IMDConvService
    {
        ...
        private async Task<bool> ConvertMarkdownCoreAsync(string inputFilePath, string outputFilePath, string format)
        {
            ...
            if (!pandocAvailable)
            {
                ...
                System.Windows.MessageBox.Show("格式转换失败：未检测到Pandoc。\n请下载并安装Pandoc以支持格式转换功能：\n下载地址: https://pandoc.org/installing.html\n安装后重启应用程序即可使用格式转换功能", "格式转换失败", System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Error);
                return false;
            }
            ...
            try
            {
                ...
                var (success, output, error) = await AppUtility.ExecuteCommandAsync("pandoc", arguments, timeoutSeconds, processedDir);
        ...
        private async Task<(bool available, string path)> CheckToolAvailabilityAsync(string toolName, string[] commandFormats, string[] commonPaths, string versionArgument = "--version")
        {
            try
            {
                var result = await AppUtility.CheckToolAvailabilityAsync(toolName, commandFormats, commonPaths, versionArgument);
                if (!result.available)
                {
        ...
        private string FixMathFormulas(string content)
        {
            var fixedContent = content;
            // 先处理块级公式，避免与行内公式冲突
            fixedContent = FixBlockMathFormulas(fixedContent);
            // 再处理行内公式
            fixedContent = FixInlineMathFormulas(fixedContent);
            return fixedContent;
        }

        private string FixBlockMathFormulas(string content)
        {
            var fixedContent = content;
            
            // 处理 \[...\] 格式（LaTex块级公式）
            var patternBrackets = @"\\\[(.*?)\\\]";
            fixedContent = Regex.Replace(fixedContent, patternBrackets, m => $"$${m.Groups[1].Value.Trim()}$$", RegexOptions.Singleline | RegexOptions.Multiline);
            
            // 处理 $$...$$ 格式（Pandoc块级公式）
            var patternDollars = @"\$\$(.*?)\$\$";
            fixedContent = Regex.Replace(fixedContent, patternDollars, m => $"$${m.Groups[1].Value.Trim()}$$", RegexOptions.Singleline | RegexOptions.Multiline);
        ...
        {
            var fixedContent = text;
            
            // 先保存块级公式，避免冲突
            var blockMarkers = new List<string>();
            fixedContent = Regex.Replace(fixedContent, @"\$\$[^$]+\$\$", m => { blockMarkers.Add(m.Value); return $"__BLOCK_MATH_{blockMarkers.Count - 1}__"; }, RegexOptions.Singleline);
            
            // 处理 \\(...\\) 格式（LaTex行内公式）
            var patternParens = @"\\\((.*?)\\\)";
            fixedContent = Regex.Replace(fixedContent, patternParens, m => $"${m.Groups[1].Value.Trim()}$", RegexOptions.Singleline | RegexOptions.Multiline);
            ...
            {
                fixedContent = fixedContent.Replace($"__BLOCK_MATH_{i}__", blockMarkers[i]);
            }
    ...
    public static class TempJsonHelper
    {
        public static async Task<string> CreateTemporaryJsonFileAsync(string inputFilePath, int fileType)
        {
            try
            {
                // 读取文件内容
                ...
                // 创建临时文件
                var tempFileName = Path.GetTempFileName() + ".json";
            ...
            {
                throw new Exception($"创建临时JSON文件失败: {ex.Message}", ex);
            }
        }
    }
}